...
public class GuaranteeActivity extends BaseActivity {
    @BindView(R.id.activity_guarantee_text)
    AppCompatTextView activity_guarantee_text;
    @Override
    ...
    public void onLoadPage(){
        ...
        Call<GetStaticPageRespons> getStaticPageResponsCall = payStaticApi.getWarrantyResponse();
        getStaticPageResponsCall.enqueue(new Callback<GetStaticPageRespons>() {
                ...
                if (getStaticPageRespons.getStatus() == true) {
                    ...
                    if (loo.equalsIgnoreCase("en")) {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                            activity_guarantee_text.setText(Html.fromHtml(getStaticPageRespons.getData().getContent_en(), Html.FROM_HTML_MODE_COMPACT));
                        } else {
                            activity_guarantee_text.setText(Html.fromHtml(getStaticPageRespons.getData().getContent_en()));
                        }
                    } else if (loo.equalsIgnoreCase("ar")) {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                            activity_guarantee_text.setText(Html.fromHtml(getStaticPageRespons.getData().getContent_ar(), Html.FROM_HTML_MODE_COMPACT));
                        } else {
                            activity_guarantee_text.setText(Html.fromHtml(getStaticPageRespons.getData().getContent_ar()));
                        }
                    }
                }
            }
            ...
            public void onFailure(Call<GetStaticPageRespons> call, Throwable t) {
                Toasty.error(GuaranteeActivity.this, getString(R.string.confirm_internet), Toast.LENGTH_LONG).show();
                reloadDialog.dismiss();
            }
        });
    }
